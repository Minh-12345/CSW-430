...
app.post('/api/users', (req, res) => {
  ...
  db.query(sql, [name, email], (err, result) => {
    if (err) res.status(500).json({ message: 'Error creating user', error: err });
    else res.status(201).json({ message: 'User created', userId: result.insertId });
  });
});
...
app.get('/api/users', (req, res) => {
  ...
  db.query(sql, (err, results) => {
    if (err) res.status(500).json({ message: 'Error fetching users', error: err });
...
app.get('/api/users/:id', (req, res) => {
  ...
  db.query(sql, [userId], (err, results) => {
    if (err) res.status(500).json({ message: 'Error fetching user', error: err });
...
app.put('/api/users/:id', (req, res) => {
  ...
  db.query(sql, [name, email, userId], (err, result) => {
    if (err) res.status(500).json({ message: 'Error updating user', error: err });
...
app.delete('/api/users/:id', (req, res) => {
  ...
  db.query(sql, [userId], (err, result) => {
    if (err) res.status(500).json({ message: 'Error deleting user', error: err });
    else if (result.affectedRows === 0) res.status(404).json({ message: 'User not found' });
